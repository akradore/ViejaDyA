...
class ReportBarcodeLabels(models.AbstractModel):
    ...
    def get_report_values(self, docids, data=None):
        ...
        config = self.env['barcode.configuration'].browse(data['form']['config_id'])#self.env.ref('dynamic_barcode_labels.default_barcode_configuration')
        if not config:
        ...
        for rec in data['form']['product_ids']:
            is_picking = False
            picking = False
            if rec.get('picking_id'):
                picking=model_picking.browse(int(rec['picking_id']))
                is_picking = True

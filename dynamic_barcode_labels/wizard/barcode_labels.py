...
class BarcodeLabels(models.TransientModel):
    ...
    @api.one
    @api.depends('product_get_ids', 'product_get_ids.qty')
    def _total_labels(self):
        self.total_labels = sum([x.qty and int(x.qty) or 0 for x in self.product_get_ids])

    td_template_id = fields.Many2one(
    ...
    total_labels = fields.Integer(
          string='Total Labels To Print',
          compute=_total_labels)
    ...
    def _create_paper_format(self, data):
        ...
        config_rec = self.td_template_id#self.env['barcode.configuration'].search([], limit=1)
        if not config_rec:
